...
class HCalDQM(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="hcal_dqm", pe_threshold=8, section=0, max_hit_time=50.0):
        ...
        # Per hit
        self.build1DHistogram(
            "pe", f"Photoelectrons in the HCal ({section_name})", *pe_bins
        )
        self.build1DHistogram(
            "hit_time", f"HCal hit time ({section_name}) [ns]", *time_bins
        )
        self.build1DHistogram(
            "sim_hit_time", f"HCal hit time ({section_name}) [ns]", *time_bins
        )
        self.build1DHistogram("layer", f"Layer number ({section_name})", *layer_bins)
        self.build1DHistogram(
            "sim_layer", f"Layer number ({section_name})", *layer_bins
        )
        self.build1DHistogram("noise", f"Is pure noise hit? ({section_name})", 2, 0, 1)
...
class HcalVetoResults(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="HcalVetoResults"):
        ...
        self.build1DHistogram("max_pe", "Maximal PE hit PE", 500, -0.5, 499.5)
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "max_pos_z", "Maximal PE hit postion Z [mm]", 6000, 200.0, 6200
        )
        self.build1DHistogram("veto_pass", "Event passed the HCal Veto", 2, -0.5, 1.5)


class HcalInefficiencyAnalyzer(ldmxcfg.Analyzer):
    def __init__(
        self,
        name="HcalInefficiencyAnalyzer",
        num_sections=5,
        pe_threshold=8,
        max_hit_time=50.0,
    ):
        ...
        section_names = ["back", "top", "bottom", "right", "left"]
        inefficiency_depth_bins = [6000, 0.0, 6000.0]
...
class EcalDigiVerify(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="EcalDigiVerify"):
        ...
        self.build1DHistogram("num_rec_hits", "Number of RecHits", 100, -0.5, 299.5)

        self.build1DHistogram(
            "num_noise_hits", "Number of noisy RecHits", 100, -0.5, 99.5
        )

        self.build1DHistogram("is_noise_hit", "Is noise hit?", 2, -0.5, 1.5)
...
class EcalShowerFeatures(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="EcalShowerFeatures"):
        ...
        self.build1DHistogram("deepest_layer_hit", "Deepest Layer Hit", 40, 0, 40)
        self.build1DHistogram("num_readout_hits", "Num Readout Hits", 100, 0, 300)
        self.build1DHistogram("summed_det", "Total Rec Energy [MeV]", 600, 0.0, 12000.0)
        self.build1DHistogram(
            "summed_iso", "Total Isolated Energy [MeV]", 600, 0.0, 12000.0
        )
        self.build1DHistogram(
            "summed_back", "Total Back Energy [MeV]", 500, 0.0, 10000.0
        )
        self.build1DHistogram(
            "max_cell_dep", "Maximum Single-Cell Energy Dep [MeV]", 200, 0.0, 2000.0
        )
        self.build1DHistogram(
            "shower_rms", "Transverse Shower RMS [mm]", 200, 0.0, 200.0
        )
        self.build1DHistogram("x_std", "X Std Deviation [mm]", 200, 0.0, 200.0)
        self.build1DHistogram("y_std", "Y Std Deviation [mm]", 200, 0.0, 200.0)
        self.build1DHistogram("avg_layer_hit", "Avg Layer Hit", 40, 0.0, 40.0)
        self.build1DHistogram("std_layer_hit", "Std Dev Layer Hit", 20, 0.0, 20.0)
        self.build1DHistogram(
...
class EcalMipTrackingFeatures(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="EcalMipTrackingFeatures"):
        ...
        self.build1DHistogram(
            "n_straight_tracks", "Num Straight Tracks", 30, -0.5, 29.5
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "ep_ang", "Electron Photon Angle [degrees]", 90, 0.0, 90.0
        )
        self.build1DHistogram("ep_sep", "Electron Photon Separation", 180, 0.0, 180.0)
        self.build1DHistogram(
            "recoil_pz", "Recoil electron p_{z} [MeV]", 200, -200.0, 8000.0
        )
        self.build1DHistogram(
            "recoil_pt", "Recoil electron p_{T} [MeV]", 200, 0, 2000.0
        )
        self.build1DHistogram("recoil_x", "Recoil electron x [mm]", 100, -300.0, 300.0)
        self.build1DHistogram("recoil_y", "Recoil electron y [mm]", 100, -300.0, 300.0)
        self.build1DHistogram("n_tracking_hits", "Num Tracking Hits", 300, 0.0, 300.0)
...
class EcalVetoResults(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="EcalVetoResults"):
        ...
        self.build1DHistogram("bdt_disc", "BDT discriminating score", 100, 0.0, 1.0)
        self.build1DHistogram(
            "bdt_disc_log", "-log(1-BDT discriminating score)", 100, 0.0, 5.0
        )
        self.build1DHistogram("fiducial", "Recoil eletron fiducial", 2, -0.5, 1.5)
        self.build1DHistogram("bdt_pass", "Event passed the ECal BDT", 2, -0.5, 1.5)
...
class EcalPnetVetoResults(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="EcalPnetVetoResults"):
        ...
        self.build1DHistogram(
            "pnet_disc", "ParticleNet discriminating score", 100, 0.0, 1.0
        )
        self.build1DHistogram(
            "pnet_disc_log", "-log(1-ParticleNet discriminating score)", 100, 0.0, 5.0
        )
        self.build1DHistogram(
            "pnet_pass", "Event passed the ECal ParticleNet", 2, -0.5, 1.5
        )
...
class VisiblesFeatureProducer(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="VisiblesFeatureProducer"):
        ...
        self.build1DHistogram(
            "total_energy", "Total Hcal energy [MeV]", 100, 4800, 9800
        )
        self.build1DHistogram(
...
class VisiblesCutflow(ldmxcfg.Analyzer):
    def __init__(self, name="VisiblesCutflow"):
        ...
        self.build1DHistogram(
            "pass_hcal_containment", "pass_hcal_containment", 40, 0, 2000
        )
        ...
        self.build1DHistogram(
            "visibles_disc_high", "visibles_disc_high", 10000, 0.999, 1
        )
        self.build1DHistogram(
...
class DarkBremInteraction(ldmxcfg.Producer):
    def __init__(self):
        ...
        self.build1DHistogram("aprime_energy", "Dark Photon Energy [MeV]", 101, 0, 8080)
        self.build1DHistogram("aprime_pt", "Dark Photon pT [MeV]", 100, 0, 2000)
        self.build1DHistogram(
            "aprime_theta", "Dark Photon Theta [degree]", 50, 0.0, 100.0
        )

        self.build1DHistogram(
            "recoil_energy", "Recoil Electron Energy [MeV]", 101, 0, 8080
        )
        self.build1DHistogram("recoil_pt", "Recoil Electron pT [MeV]", 100, 0, 2000)
        self.build1DHistogram(
            "recoil_theta", "Recoil Electron Theta [degree]", 50, 0.0, 100.0
        )

        self.build1DHistogram(
            "incident_energy", "Incident Electron Energy [MeV]", 101, 0, 8080
        )
        self.build1DHistogram("incident_pt", "Incident Electron pT [MeV]", 100, 0, 2000)

        # weird binning so we can see the target and trigger pads
        self.build1DHistogram(
            "dark_brem_z", "Z Location of Dark Brem [mm]", 160, -7.0, 1.0
        )
...
class PhotoNuclearDQM(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="PN", count_light_ions=True):
        ...
        self.build1DHistogram(
            "event_type_compact_500mev", "", event_type_compact_labels
        )
        self.build1DHistogram(
            "event_type_compact_2000mev", "", event_type_compact_labels
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "pn_particle_mult", "Photo-nuclear Multiplicity", 200, 0, 200
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "1n_neutron_energy", "Neutron Energy [MeV]", 100, 0, 10000
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "1kp_energy_frac", "E(K#pm)/E(#gamma_{PN}) [MeV]", 100, 0, 1
        )
        ...
        self.build1DHistogram(
            "1k0_energy_frac", "E(K0)/E(#gamma_{PN}) [MeV]", 100, 0, 1
        )

        self.build1DHistogram(
            "recoil_vertex_x", "Recoil e^{-} Vertex - x [mm]", 40, -40, 40
        )
        self.build1DHistogram(
            "recoil_vertex_y", "Recoil e^{-} Vertex - y [mm]", 80, -80, 80
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "pn_gamma_vertex_x", "#gamma Vertex - y [mm]", 40, -40, 40
        )
        self.build1DHistogram(
            "pn_gamma_vertex_y", "#gamma Vertex - y [mm]", 80, -80, 80
        )
...
class TrkDeDxMassEstFeatures(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="TrkDeDxMassEstFeatures"):
        ...
        self.build1DHistogram("harmonic_mean_dedx", "I_{h} [MeV/cm]", 50, 0.0, 30.0)
        self.build1DHistogram("mass_estimate", "Mass Estimate [MeV]", 100, 0.0, 2000.0)
        self.build1DHistogram(
            "mass_estimate_low_p", "Mass Estimate [MeV]", 100, 0.0, 2000.0
        )
        self.build1DHistogram(
...
class TrigScintClusterDQM(ldmxcfg.Analyzer):
    ...
    def __init__(
        self, name="TrigScintClusterUp", coll="TriggerPadUpClusters", pad="up"
    ):
...
class SampleValidation(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="SampleValidation"):
        ...
        # primary histograms
        self.build1DHistogram(
            "primaries_pdgid", "ID of primary particles", pdgid_bin_labels
        )
        self.build1DHistogram(
            "primaries_energy", "Energy of primary particles [MeV]", 90, 0, 9000
        )  # range applicable for 4 GeV beam
        self.build1DHistogram("primaries_theta", "Theta [degree]", 50, 0.0, 100.0)
        self.build1DHistogram(
            "primaries_pt", "Transverse Momentum [MeV]", 200, 0.0, 2000.0
        )

        self.build2DHistogram(
            "beam_smear", "x [mm]", 30, -150, 150, "y [mm]", 30, -150, 150
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram("harddaughters_theta", "Theta [degree]", 25, 0.0, 50.0)
        self.build1DHistogram(
            "harddaughters_pt", "Transverse Momentum [MeV]", 50, 0.0, 100.0
        )
        ...
        self.build1DHistogram(
            "hardbremdaughters_theta", "Theta [degree]", 50, 0.0, 100.0
        )
        self.build1DHistogram(
...
class EcalClusterAnalyzer(ldmxcfg.Analyzer):
    ...
    def __init__(self, name="EcalClusterAnalyzer"):
        ...
        self.build1DHistogram("ancestors", "Ancestors of particles", 4, 0.0, 4.0)

        self.build1DHistogram(
            "same_ancestor",
            "Percentage of hits in cluster coming from the electron that produced most hits",
            21,
            0.0,
            105.0,
        )
        self.build1DHistogram(
            "energy_percentage",
            "Percentage of energy in cluster coming from the electron that produced most of energy",
            21,
            0.0,
            105.0,
        )
        self.build1DHistogram(
            "mixed_hit_energy",
            "Percentage of total energy coming from hits with energy contributions from more than one electron",
            21,
            0.0,
            105.0,
        )
        self.build1DHistogram(
            "clusterless_hits", "Number of hits not in a cluster", 10, 0.0, 200.0
        )
        self.build1DHistogram(
        ...
        self.build1DHistogram(
            "total_rechits_in_event", "Number of RecHits", 20, 0.0, 500.0
        )
...
hcal_dqm = [
    HCalDQM(pe_threshold=8, section=0),
    HCalDQM(pe_threshold=8, section=1),
    HCalDQM(pe_threshold=8, section=2),
    HCalDQM(pe_threshold=8, section=3),
    HCalDQM(pe_threshold=8, section=4),
